...
userRouter.get("/all", async (req,res)=>{
    try{
        ...
        res.status(400).send({message:"Something went wrong to fetch User Data",error:error.message})
        console.log(error)
    }
})
...
userRouter.get("/singletrainer/:id", async (req,res)=>{
    ...
    try{
        ...
        res.status(400).send({message:"Something went wrong.. Trainers Data can not Fetched",error:error.message})
        console.log(error)
    }
})
...
userRouter.get("/:id", async (req,res)=>{
    ...
    try{
        ...
        res.status(400).send({message:"Something went wrong to fetch data by id",error:error.message})
        console.log(error)
    }
})
    ...
    try{
        ...
        if(user.length>0){
            ...
            bcrypt.hash(password, + process.env.salt, async function(err, hash) {
                if(err){
                    res.status(401).send({message:"Server Error happens",error:err.message});
                    console.log(err)
                }else{
...
userRouter.post("/login", async (req,res)=>{
    ...
    try{
        ...
        if(!user){
            res.status(400).send({error:"User not found here, please register", "OK" :  false})
        }else{
            bcrypt.compare(password, user.password, async function(err, result) {
                if(result){
                    ...
                    if(user.role=="trainer"){
                        res.status(200).send({message:" Now Trainer Logged In",token,refresh_token,user, "OK": true})
                    }else{
                        res.status(200).send({message:" Now User Logged In",token,refresh_token,user, "OK": true})
                    }
                }else{
                    res.status(401).send({error:"You have Inter Incorrect Password, Kindly Login Again", "OK": false});
                    console.log(err)
                }
            });            
        }
...
userRouter.patch("/update/:id", async (req,res)=>{
    ...
    try{
        ...
        res.status(400).send({message:"Something went wrong to update the data",error:error.message})
        console.log(error)
    }
})
...
userRouter.delete("/delete/:id", async (req,res)=>{
    ...
    try{
        ...
        res.status(400).send({message:"Something went wrong to delete user",error:error.message})
        console.log(error)
    }
})
